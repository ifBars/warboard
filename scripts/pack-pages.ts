...
const bytes = (await Promise.all((await files("dist")).map(async p => (await stat(p)).size))).reduce((a,b) => a+b, 0);
console.log(`Pages artifact: ${(bytes / 1024 / 1024).toFixed(1)} MiB`);
if (bytes > 950_000_000) throw Error("Pages artifact exceeds the 950 MB deployment budget.");
